...
function getAllPhones(personID) {
  return fetch(URL+"phonelist/"+personID)
  .then(handleHttpErrors)
}

function addPhoneNumber(phone, personID) {
  let options = makeOptions("POST", phone);
  return fetch(URL+"addphone/"+personID,options)
  .then(handleHttpErrors)
}

const personFacade = {
  //Object with functions in it
  getAllPeople,
  addPerson,
  editPerson,
  deletePerson,
